function [U,x,t]=implicitoOndas(CC1,CC2,CI1,CI2,a,b,nx,nt,Tmax,alfa) 
...
for i=2:nx 
    U(i,2) = (1-lambda2)*fx(i) +lambda2/2*(fx(i+1)+fx(i-1))+k*gx(i); 
end
...
for j=2:nt 
    ...
    d(1)=d(1)+lambda2/2*(U(1,j+1)+U(1,j-1));
    d(end)=d(end)+lambda2/2*(U(nx+1,j+1)+U(nx+1,j-1));
