function [u,x,t] = CalorCN_Entr3_3(alpha,ci,h2,L,nx,Tmax,nt) 
...
%     disp('sin problema') 
% end
dp=(1+lambda-k/2)*ones(nx,1);
ds=(-lambda/2+lambda*h/4)*ones(nx-1,1);
ds(1)=-lambda;
di=(-lambda/2-lambda*h/4)*ones(nx-1,1);

dpB=(1-lambda+k/2)*ones(nx,1);
dsB=(lambda/2-lambda*h/4)*ones(nx-1,1);
dsB(1)=lambda;
diB=(lambda/2+lambda*h/4)*ones(nx-1,1);
