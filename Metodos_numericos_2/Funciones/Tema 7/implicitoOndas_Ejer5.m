function [U,x,t]=implicitoOndas_Ejer5(CC1,CC2,CI1,CI2,a,b,nx,nt,Tmax,alfa) 
...
for j=2:nt 
    d=6*U(2:nx,j)+k*lambda*(U(3:nx+1,j)-U(1:nx-1,j))+B*U(2:nx,j-1)-k^2*cos(pi*x(2:nx)); 
    d(end)=d(end)+lambda2*(t(j+1)+t(j-1));
