function [U,x,t] = explicitoOndas_Ejer4(CC1,CC2,CI1,CI2,a,b,nx,nt,Tmax,alfa) 
...
for j=2:nt 
    for i=2:nx 
        U(i,j+1)=((2-2*lambda2+k^2)*U(i,j)+lambda2*(U(i+1,j)+U(i-1,j))-U(i,j-1)*(1+t(j)*k/2))/(1-k*t(j)/2); 
    end 
end 
end