function [U,x,t] = explicitoOndas_Ej2(CC1,CC2,CI1,CI2,a,b,nx,nt,Tmax,alfa) 
...
for i=2:nx 
    U(i,2) = (1-lambda2-k^2/2)*sin(pi*x(i)) +lambda2/2*(sin(pi*x(i+1))+sin(pi*x(i-1))); 
end 
for j=2:nt 
    for i=2:nx 
        U(i,j+1)=2*(1-lambda2-k^2/2)*U(i,j)+lambda2*(U(i+1,j)+U(i-1,j))-U(i,j-1); 
    end 
end 
end