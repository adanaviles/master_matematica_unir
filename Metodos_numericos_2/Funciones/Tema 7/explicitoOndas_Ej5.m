function [U,x,t] = explicitoOndas_Ej5(CC1,CI1,CI2,a,b,nx,nt,Tmax,alfa) 
...
for j=2:nt 
    for i=2:nx 
        U(i,j+1)=2*(1-lambda2)*U(i,j)+lambda2*(U(i+1,j)+U(i-1,j))-U(i,j-1)+k^2*exp(-t(j)); 
        U(nx+1,j+1)=2*(1-lambda2)*U(nx+1,j)+2*lambda2*U(nx,j)-U(nx+1,j-1)-6*h*lambda2*cos(t(j))+k^2*exp(-t(j)); 
    end 
end 
end