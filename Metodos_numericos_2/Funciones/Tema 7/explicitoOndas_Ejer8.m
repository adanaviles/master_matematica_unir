function [U,x,t] = explicitoOndas_Ejer8(CC1,CI1,CI2,a,b,nx,nt,Tmax,alfa) 
...
for i=2:nx 
    U(i,2) = (1-lambda2)*fx(i) +lambda2/2*(fx(i+1)+fx(i-1))+k*gx(i)+k^2*sin(x(i))/2; 
end 
for j=2:nt 
    for i=2:nx 
        U(i,j+1)=2*(1-lambda2)*U(i,j)+lambda2*(U(i+1,j)+U(i-1,j))-U(i,j-1)+k^2*sin(x(i)); 
    end
    U(nx+1,j+1)=2*(1-lambda2)*U(nx+1,j)+lambda2*(U(nx,j)+U(nx,j))-U(nx+1,j-1)+k^2*sin(x(i));
end 
end