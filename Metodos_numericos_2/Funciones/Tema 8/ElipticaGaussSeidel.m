function [U,incre,iter,x,y]=ElipticaGaussSeidel(f0,f1,g0,g1,a,b,c,d,nx,ny,funcionti,maxiter,tol) 
...
while iter<maxiter && incre>tol 
    for j=2:nx 
        for i=2:ny 
            V(i,j)=q*(U(i+1,j)+V(i-1,j))+ p*(U(i,j+1)+V(i,j-1))+ r*feval(funcionti,x(j),y(i)); 
        end 
    end 
