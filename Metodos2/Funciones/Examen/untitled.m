...
function [X,Y,iter,incr]=DifnolinEX1_b(f,fy,fz,a,b,alfa,beta,N,maxiter,tol) 
...
while incr>tol && iter<maxiter
    ...
    % centrales
    % Si tengo condiciones no dirichlet, tengo que tener en cuenta esta
    % variacion, OJITO
    z=[alfa+Y(1) z -Y(N+1)+beta];
    fe=feval(f,x,y,z); 
    fye=feval(fy,x,y,z); 
    fze=feval(fz,x,y,z);
    ...
    dp(end)=2*(1+h)+h^2*fye(end)-h^2*fze(end); 
    ...
    d(1)=2*(1+h)*y(1)-2*y(2)+h^2*fe(1)+h;
    d(1)=-d(1);
    d(end)=2*(1+h)*y(N+1)-2*y(N+1)+h^2*fe(N+1)-17*h;
    d(end)=-d(end);
...
while incr>tol && iter<maxiter
    z=[alfa+Y(1) (Y(3:N+2)-Y(1:N))/(2*h) beta-Y(N+2)]; % Estimacion de la derivada por diferencias
    % centrales
