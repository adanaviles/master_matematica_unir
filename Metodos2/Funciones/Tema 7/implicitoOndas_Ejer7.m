function [U,x,t]=implicitoOndas_Ejer7(CI1,CI2,a,b,nx,nt,Tmax,alfa) 
...
for j=2:nt 
    d=2*U(:,j)+B*U(:,j-1);
    d(2:end-1)=d(2:end-1)+k*lambda/2*(U(3:end,j)-U(1:end-2,j));
    d(end)=d(end)+k*h*lambda*U(end,j);
