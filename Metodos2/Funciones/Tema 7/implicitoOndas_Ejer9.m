function [U,x,t]=implicitoOndas_Ejer9(CI1,CI2,a,b,nx,nt,Tmax,alfa) 
...
dpa=(pi^2+1+lambda2)*ones(nx+1,1); 
dpa(1)=dpa(1)-h*lambda2/2; dpa(end)=dpa(end)-pi*h*lambda2;
...
for j=2:nt 
    ...
    d(end)=d(end)-pi*h*lambda2*(sin(t(j+1))+sin(t(j-1)));
